...
async def process_add_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("awaiting_group_add"):
        ...
        try:
            # Prüfe, ob die Spalte existiert, wenn nicht, erstelle sie
            cursor.execute("PRAGMA table_info(allowed_groups);")
            ...
            # Gruppe hinzufügen oder aktualisieren
            cursor.execute(f"""
            ...
            await update.message.reply_text(f"⚠️ Fehler beim Einfügen: {e}")
...
async def process_remove_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("awaiting_group_remove"):
        ...
        try:
            ...
            await update.message.reply_text(f"⚠️ Fehler beim Entfernen: {e}")
