...
async def get_group_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("📋 In Zwischenablage kopieren", callback_data=f"copy_{chat_id}")]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await update.message.reply_text(f"📌 **Gruppen-ID:** `{chat_id}`", parse_mode="Markdown", reply_markup=reply_markup)

# --- Callback-Funktion für den "Kopieren"-Button ---
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    chat_id = query.data.split("_")[1]  # Die Gruppen-ID extrahieren
...
def main():
    ...
    # Callback für den "Kopieren"-Button
    application.add_handler(CommandHandler("copy_id", button_callback))
